...
{
#ifdef AK_OS_WCHAR
	AkInt32 msgErrorLength = AkWideCharToChar(in_pszError, 0, NULL);
	char* c_pszError = (char*)AkAlloca(msgErrorLength);
	AkWideCharToChar(in_pszError, msgErrorLength, c_pszError);
	if (in_eErrorLevel == AK::Monitor::ErrorLevel_Error && Instance().m_pMenu->GetCurrentPage())
		Instance().m_pMenu->GetCurrentPage()->SetErrorMessage(c_pszError);
	AKPLATFORM::OutputDebugMsgV("%ls: %s\n",
		in_eErrorLevel == AK::Monitor::ErrorLevel_Message ? AKTEXT("AK Message") : AKTEXT("AK Error"),
		c_pszError);
#else
	if (in_eErrorLevel == AK::Monitor::ErrorLevel_Error && Instance().m_pMenu->GetCurrentPage())
		Instance().m_pMenu->GetCurrentPage()->SetErrorMessage(in_pszError);
