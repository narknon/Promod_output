...
const char* s_labelJobThreads= "Job Threads:";
...
void DemoOptions::InitJobWorkerSettings()
{
	...
	AK::JobWorkerMgr::InitSettings& workerSettings = IntegrationDemo::Instance().GetJobWorkerSettings();
	ToggleControl* newToggle = new ToggleControl(*this);
	newToggle->SetLabel(s_labelJobThreads);

	static_assert(AK_MAX_WORKER_THREADS == 4, "Please update the options to fix the constant.");
	newToggle->AddOption("Disabled", (void*)0);
	newToggle->AddOption("1 thread", (void*)(1));
	newToggle->AddOption("2 threads", (void*)(2));
	newToggle->AddOption("3 threads", (void*)(3));
	newToggle->AddOption("4 threads", (void*)(4));
	newToggle->SetSelectedIndex(workerSettings.uNumWorkerThreads);
...
void DemoOptions::InitSettings_Changed(void* in_pSender, ControlEvent* in_pControlEvent)
{
	...
	workerSettings.uNumWorkerThreads = (AkUInt32)(AkUIntPtr)((ToggleControl*)FindControl(s_labelJobThreads))->SelectedValue();
	initSettings.settingsJobManager = workerSettings.GetJobMgrSettings();
#endif
