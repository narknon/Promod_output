	...
	, m_uCurrentPlayingID(AK_INVALID_PLAYING_ID)
{
	m_szHelp =
		...
		"When the page is loaded, a PrepareBank operation loads the lightweight structure and events data "
		"referenced by this demo, without loading any actual media into memory.\n\n"
		"When the user moves the cursor to an area button, a PrepareEvent operation will "
		"load the corresponding media file (a .wem file on disk) into memory in anticipation of a future PostEvent.\n\n" 
		"When the user finally enters the area, the event is posted and the media is already ready to play. ";
}
