...
void DemoSpatialAudio::Release()
{
	...
	if (m_bDistanceProbeRegistered)
	{
		AK::SoundEngine::UnregisterGameObj(DISTANCE_PROBE);
		m_bDistanceProbeRegistered = false;
	}
...
int DemoSpatialAudio::AKPosToPixels_X(double in_X)
{
	return (int)(((in_X / POSITION_RANGE) + 0.5f) * m_fWidth);
}

int DemoSpatialAudio::AKPosToPixels_Y(double in_y)
{
...
void DemoSpatialAudio::UpdateGameObjPos(MovableChip* in_pChip, AkGameObjectID in_GameObjectId)
{
	if (in_pChip)
	{
		...
		orientationFront.Y = 0;
		orientationFront.X = 0;
		AkVector orientationTop;
		orientationTop.X = 0;
		orientationTop.Y = 1;
		orientationTop.Z = 0;

		if (in_GameObjectId == LISTENER)
		{
			orientationFront.X = sinf(m_emitterAz) * cosf(m_emitterEl);
			orientationFront.Y = sinf(m_emitterEl);
			orientationFront.Z = cosf(m_emitterAz) * cosf(m_emitterEl);

			orientationTop.X = sinf(m_emitterEl) * -sinf(m_emitterAz);
			orientationTop.Y = cosf(m_emitterEl);
			orientationTop.Z = -sinf(m_emitterEl) * cosf(m_emitterAz);

			if (m_listenerPlayerOffset > 0.f)
			{
				if (!m_bDistanceProbeRegistered)
				{
					AK::SoundEngine::RegisterGameObj(DISTANCE_PROBE, "Distance Probe");
					AK::SoundEngine::SetDistanceProbe(LISTENER, DISTANCE_PROBE);
					m_bDistanceProbeRegistered = true;
				}

				AkSoundPosition soundPos;
				soundPos.Set(position, orientationFront, orientationTop);
				AK::SoundEngine::SetPosition(DISTANCE_PROBE, soundPos);

				AkRoomID distanceProbeRoomID = IsInRoom((float)AKPosToPixels_X(position.X), (float)AKPosToPixels_Y(position.Z));
				AK::SpatialAudio::SetGameObjectInRoom(DISTANCE_PROBE, distanceProbeRoomID);

				position.X -= orientationFront.X * m_listenerPlayerOffset;
				position.Y -= orientationFront.Y * m_listenerPlayerOffset;
				position.Z -= orientationFront.Z * m_listenerPlayerOffset;
			}
			else
			{
				if (m_bDistanceProbeRegistered)
				{
					AK::SoundEngine::UnregisterGameObj(DISTANCE_PROBE);
					AK::SoundEngine::SetDistanceProbe(LISTENER, AK_INVALID_GAME_OBJECT);
					m_bDistanceProbeRegistered = false;
				}
		...
		// For SpatialAudio
		AkRoomID roomID = IsInRoom((float)AKPosToPixels_X(position.X), (float)AKPosToPixels_Y(position.Z));
	...
	for ( it = m_pParentMenu->Input()->Begin(); it != m_pParentMenu->Input()->End(); it++ )
	{
		...
		if ((*it).IsButtonDown(UG_BUTTON3))
		{
			if ((*it).IsButtonDown(UG_BUTTON5))
				m_emitterEl -= kAngleIncrement;
			else
				m_emitterAz -= kAngleIncrement;
		...
		if (m_uRepeat == 0)
		{
			...
			if ((*it).IsButtonDown(UG_BUTTON6))
			{
				m_listenerPlayerOffset += 5.f;
				if (m_listenerPlayerOffset > 20.f)
					m_listenerPlayerOffset = 0.f;
...
GeometryInRooms::GeometryInRooms(DemoSpatialAudio * in_pDemo)
{
...

	{
		...
		*/
		...
		AK::SpatialAudio::SetGeometryInstance(GEOMETRY_ROOM, instanceParams);
	}

	{
		...
		*/
		...
		AkGeometryInstanceParams instanceParams;
		instanceParams.GeometrySetID = 1;

		// Associate with room "Outside". Doing so, the edges used by geometric diffraction are only searched when emitters or the listener are in this room, and the search space is limited to this room, 
		// which is much quicker than searching across the edges of all geometry. Coupling between edge diffraction of separate rooms is handled by portal diffraction, which is less CPU intensive.
		instanceParams.RoomID = ROOM;

		AK::SpatialAudio::SetGeometryInstance(1, instanceParams);
	}
...

	{
		...
		AkGeometryInstanceParams instanceParams;
		instanceParams.GeometrySetID = 2;

		// Associate with room "Outside". Doing so, the edges used by geometric diffraction are only searched when emitters or the listener are in this room, and the search space is limited to this room, 
		// which is much quicker than searching across the edges of all geometry. Coupling between edge diffraction of separate rooms is handled by portal diffraction, which is less CPU intensive.
		instanceParams.RoomID = AK::SpatialAudio::kOutdoorRoomID;

		AK::SpatialAudio::SetGeometryInstance(2, instanceParams);
	}
