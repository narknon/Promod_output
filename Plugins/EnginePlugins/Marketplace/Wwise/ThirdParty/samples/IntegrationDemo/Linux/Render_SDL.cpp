...
namespace sdlCore
{
	...
	AkUInt32 g_fRenderWidth = 0;
	AkUInt32 g_fRenderHeight = 0;
	...
	bool Init(void* in_pParam, AkOSChar* in_szErrorBuffer, unsigned int in_unErrorBufferCharCount)
	{
		...
		{
			if (in_szErrorBuffer)
				__AK_OSCHAR_SNPRINTF(in_szErrorBuffer, in_unErrorBufferCharCount, AKTEXT("Renderer could not be created! SDL Error: %s\n"), SDL_GetError());
			return false;
		}

		int width, height;
		SDL_GetWindowSize((SDL_Window*)(in_pParam), &width, &height);
		g_fRenderWidth = (float)width;
		g_fRenderHeight = (float)height;
		return true;
	}
	...
	void FlushResourceDestructionQueue()
	{
		...
		{
			for (int i = 0; i < g_textureDestructionQueue.size(); ++i)
			{
...
public:
	...
	void UpdateResource(const void* buffer, size_t bufferPitch, size_t bufferLength)
	{
		...
		AKASSERT(bufferPitch <= texturePitch);
...
namespace drawTexture2D
{
	...
	void ExecuteCommands()
	{
		...
		for (std::vector<DrawCommand>::iterator commandIt = commands.begin(); commandIt != commands.end(); ++commandIt)
		{
			...
			int x1 = commandIt->dest.x1 * sdlCore::g_fRenderWidth;
			int x2 = commandIt->dest.x2 * sdlCore::g_fRenderWidth;
			int y1 = commandIt->dest.y1 * sdlCore::g_fRenderHeight;
			int y2 = commandIt->dest.y2 * sdlCore::g_fRenderHeight;
			...
			x1 = commandIt->src.x1 * texWidth;
			x2 = commandIt->src.x2 * texWidth;
			y1 = commandIt->src.y1 * texHeight;
			y2 = commandIt->src.y2 * texHeight;
	...
	void ExecuteCommands()
	{
		for (std::vector<DrawCommand>::iterator commandIt = commands.begin(); commandIt != commands.end(); ++commandIt)
		{
			SDL_SetRenderDrawColor(sdlCore::g_pRenderer,
				commandIt->color[0] * 255,
				commandIt->color[1] * 255,
				commandIt->color[2] * 255,
				255);
			SDL_RenderDrawLine(sdlCore::g_pRenderer,
				sdlCore::g_fRenderWidth * commandIt->x1,
				sdlCore::g_fRenderHeight * commandIt->y1,
				sdlCore::g_fRenderWidth * commandIt->x2,
				sdlCore::g_fRenderHeight * commandIt->y2);
		}
		// consumed the commands, so clear them out
		commands.clear();
	}
