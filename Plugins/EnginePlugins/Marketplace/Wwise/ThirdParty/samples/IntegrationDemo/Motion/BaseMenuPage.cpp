...
void BaseMenuPage::Draw()
{
	Page::Draw();

#if (defined AK_WIN && !defined AK_USE_UWP_API)
	AkInt32 xPos, yPos;
	xPos = (AkInt32)(m_pParentMenu->GetWidth() * 0.1);
	yPos = m_pParentMenu->GetHeight() - 5 * GetLineHeight(DrawStyle_Text);
#if defined(INTDEMO_LIBSCEPAD_SUPPORT)
	DrawTextOnScreen("SCE Gamepad support is available", 
		xPos, yPos, DrawStyle_Text);
#else
	DrawTextOnScreen("SCE Gamepad support is not available.\nDefine INTDEMO_LIBSCEPAD_SUPPORT if supporting libraries are available.", 
		xPos, yPos, DrawStyle_Text);
#endif // defined(INTDEMO_LIBSCEPAD_SUPPORT)
