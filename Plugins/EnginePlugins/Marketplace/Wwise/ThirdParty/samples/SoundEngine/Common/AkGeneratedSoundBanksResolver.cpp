	...
	if (in_pFlags->uCompanyID == AKCOMPANYID_AUDIOKINETIC)
	{
		if (in_bUseSubfoldering)
		{
			...
			case AkFileKind_BusBank:
			{
				...
				AK_OSPRINTF(szSubfolder, 12, AKTEXT("%u"), (unsigned int)in_pFlags->uDirectoryHash);

				// This might look weird but it is to
				// prevent a Clang 14 bug when assigning zero
				// to a stack char buffer.
				AkOSChar szTmp[3] = {};
				szTmp[0] = szSubfolder[0];
				szTmp[1] = szSubfolder[1];
				dest.Append(szTmp, 2);
				dest.Append(AK_PATH_SEPARATOR, uSeparatorLen);
			}
