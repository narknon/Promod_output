...
bool PerforceSourceControl::ShowConfigDlg()
{
	...
	Error error;
	if (uiResult == IDOK && !InitClient(&error))
	{
...
{	
	...
	ISourceControl::OperationResult eResult = GetFileStatus( in_rFilenameList, &out_rFileStatusMap, true, INFINITE );
	if ( eResult == OperationResult_Succeed )
	{
		out_rFilenameList.Reserve(in_rFilenameList.GetSize());
		...
		for ( ;filePos; in_rFilenameList.GetNext( filePos ) )
		{
			...
			if( IsOperationValid( (OperationID)in_dwOperationID, fileStatus, czFilename, false ) )
			{
...
{	
	// First get the file statuses
	ISourceControl::OperationResult eResult = GetFileStatus( in_rFilenameList, NULL, true, INFINITE );
	if ( eResult == OperationResult_Succeed )
	{
		// Get files that are not checked out
		SourceControlContainers::AkPos filePos = in_rFilenameList.GetHeadPosition();
...
{
	ISourceControl::OperationResult eResult = GetFileStatus( in_rFilenameList, NULL, true, INFINITE, false );
	if ( eResult == OperationResult_Succeed )
	{
		// Get files that are not checked out
		SourceControlContainers::AkPos filePos = in_rFilenameList.GetHeadPosition();
