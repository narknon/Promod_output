...
AK::Wwise::ISourceControl::OperationResult SamplePlugin::CheckFilesForOperation( DWORD in_dwOperationID, const StringList& in_rFilenameList, BooleanList& out_rFileStatusList )
{
	for ( unsigned int i = 0; i < in_rFilenameList.GetCount(); ++i )
	{
		out_rFileStatusList.AddTail( true );
	}

	return OperationResult_Succeed;
}

AK::Wwise::ISourceControl::OperationResult SamplePlugin::FilesUnderSourceControl( const StringList& in_rFilenameList, BooleanList& out_rFileStatusList )
{
	for ( unsigned int i = 0; i < in_rFilenameList.GetCount(); ++i )
	{
		out_rFileStatusList.AddTail( false );
	}
...
ISourceControl::OperationResult SamplePlugin::GetFileStatusAndIcons(
	...
	DWORD in_dwTimeoutMs)
{
	...
	GetFileStatus(in_rFilenameList, out_rFileStatusMap, in_dwTimeoutMs);
