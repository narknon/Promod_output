...
void SubversionSourceControl::GetPluginInfo( PluginInfo& out_rPluginInfo )
{
	...
	out_rPluginInfo.m_dwCheckOutNoUICommandID = AK::Wwise::SourceControlConstant::s_dwInvalidOperationID;
...
ISourceControl::OperationResult SubversionSourceControl::GetFilesForOperation( DWORD in_dwOperationID, const StringList& in_rFilenameList, StringList& out_rFilenameList, FilenameToStatusMap& out_rFileStatusMap )
{	
	...
	if ( eResult == OperationResult_Succeed )
	{
		...
		out_rFilenameList.Reserve(statusList.size());
		for (const auto& status : statusList)
		{
			LPCWSTR szFile = in_rFilenameList.GetNext( filePos );

			if( IsOperationValid( (OperationID)in_dwOperationID, status, false ) )
			{
...
ISourceControl::OperationResult SubversionSourceControl::CheckFilesForOperation( DWORD in_dwOperationID, const StringList& in_rFilenameList, BooleanList& out_rFileStatusList )
{
	SvnStatusList statusList;
	ISourceControl::OperationResult eResult = _GetFileStatus( in_rFilenameList, NULL, INFINITE, &statusList );
	if ( eResult == OperationResult_Succeed )
	{
		// Fill list
		for (const auto& status : statusList)
		{
			bool bOpOk = IsOperationValid( (OperationID)in_dwOperationID, status, false );
...
ISourceControl::OperationResult SubversionSourceControl::FilesUnderSourceControl( const StringList& in_rFilenameList, BooleanList& out_rFileStatusList )
{
	SvnStatusList statusList;
	ISourceControl::OperationResult eResult = _GetFileStatus( in_rFilenameList, NULL, INFINITE, &statusList );
	if ( eResult == OperationResult_Succeed )
	{
		...
		{
			bool bVersioned =
				status.m_kind != svn_wc_status_ignored &&
				status.m_kind != svn_wc_status_none &&
				status.m_kind != svn_wc_status_unversioned ;

			out_rFileStatusList.AddTail( bVersioned );
		}
	}
...
ISourceControl::OperationResult SubversionSourceControl::GetOperationList( OperationMenuType in_menuType, const StringList& in_rFilenameList, OperationList& out_rOperationList )
{
	...
	SvnStatusList statusList;
	statusList.reserve(in_rFilenameList.GetSize());
...
ISourceControl::OperationResult SubversionSourceControl::_GetFileStatus( const StringList& in_rFilenameList, FilenameToStatusMap* out_pFileStatusMap, DWORD in_dwTimeoutMs, SvnStatusList* out_pStatuses )
{
	AFX_MANAGE_STATE( AfxGetStaticModuleState() );
	...
	if( out_pStatuses )
		out_pStatuses->reserve(in_rFilenameList.GetSize());
	...
	for ( SourceControlContainers::AkPos filePos = in_rFilenameList.GetHeadPosition() ; filePos ; )
	{
		...
		if( out_pStatuses )
			out_pStatuses->push_back( status );
		...
		if( out_pFileStatusMap )
		{
			...
			out_pFileStatusMap->SetAt( csFilename, statusItem );
		}
	}
...
ISourceControl::OperationResult SubversionSourceControl::GetFileStatusAndIcons(
	...
	const StringListView& in_rFilenameList)
{
	...
	switch( in_dwOperationID )
	{
		...
		case OperationID_AddNoUI:
			Add( in_rFilenameList, in_dwOperationID == OperationID_Add );
			break;
		...
		case OperationID_RevertNoUI:
			Revert( in_rFilenameList, in_dwOperationID == OperationID_Revert );
			break;
...
void SubversionSourceControl::Update(const StringListView& in_rFilenameList)
{
...
void SubversionSourceControl::Commit(const StringListView& in_rFilenameList)
{
...
void SubversionSourceControl::GetLock(const StringListView& in_rFilenameList)
{
...
void SubversionSourceControl::ReleaseLock(const StringListView& in_rFilenameList)
{
	m_pUtilities->GetProgressDialog()->ShowProgress();
...
void SubversionSourceControl::Add(const StringListView& in_rFilenameList, bool in_bShowUI)
{
	if (in_bShowUI)
		m_pUtilities->GetProgressDialog()->ShowProgress();
	...
	for (const CString csFilename : in_rFilenameList)
	{
...
AK::Wwise::ISourceControl::IOperationResult* SubversionSourceControl::Delete(const StringListView& in_rFilenameList, bool in_bShowUI)
{
	AFX_MANAGE_STATE( AfxGetStaticModuleState() );
	...
	localOnlyFilenameList.reserve(in_rFilenameList.size());
	serverFilenameList.reserve(in_rFilenameList.size());
	...
	for (const CString csFilename : in_rFilenameList)
	{
		...
		{
			serverFilenameList.push_back( csFilename );
		}
	}
	...
	if (!localOnlyFilenameList.empty())
	{
		...
		for (const CString csFilename : localOnlyFilenameList)
		{
			bool bSuccess = false;
...
AK::Wwise::ISourceControl::IOperationResult* SubversionSourceControl::Rename(const StringListView& in_rFilenameList)
{
	FileOperationResult* pResult = NULL;

	AFX_MANAGE_STATE(AfxGetStaticModuleState());

	ASSERT( in_rFilenameList.size() == 1 );

	const auto& csOldFilename = in_rFilenameList[0];
...
AK::Wwise::ISourceControl::IOperationResult* SubversionSourceControl::MoveNoUI( DWORD in_dwOperationID, const StringListView& in_rFilenameList, const StringListView& in_pTargetFilenameList )
{
	ASSERT(in_pTargetFilenameList.size() == in_rFilenameList.size());
	if (in_pTargetFilenameList.size() != in_rFilenameList.size())
		return nullptr;
...
AK::Wwise::ISourceControl::IOperationResult* SubversionSourceControl::Move(const StringListView& in_rFilenameList)
{
	FileOperationResult* pResult = NULL;
...
void SubversionSourceControl::Revert(const StringListView& in_rFilenameList, bool in_bShowUI)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
void SubversionSourceControl::Resolved(const StringListView& in_rFilenameList)
{
	m_pUtilities->GetProgressDialog()->ShowProgress();
...
void SubversionSourceControl::Diff(const StringListView& in_rFilenameList)
{
	ASSERT( in_rFilenameList.size() == 1 );

	const CString csFilename = in_rFilenameList[0];
...
void SubversionSourceControl::ShowLog(const StringListView& in_rFilenameList)
{
	ASSERT( in_rFilenameList.size() == 1 );
	...
	if ( in_eOperation & CreateOrModifyOperation_Create )
	{
		...
		for ( SourceControlContainers::AkPos filePos = in_rFilenameList.GetHeadPosition() ; filePos ; )
		{
			...
			if ( status.m_kind == svn_wc_status_none )
				m_filesToAdd.push_back( csFilename );

			svn_error_clear( pError );
		}

		if ( !m_filesToAdd.empty() )
		{
...
ISourceControl::OperationResult SubversionSourceControl::PostCreateOrModify( const StringList& in_rFilenameList, CreateOrModifyOperation in_eOperation, bool& out_rContinue )
{
	...
	if ( ( in_eOperation & CreateOrModifyOperation_Create ) && m_bOnNewFilesAdd && !m_filesToAdd.empty() )
	{
...
apr_array_header_t* SubversionSourceControl::GetSVNPathArray( const StringListView& in_rPathList, apr_pool_t* in_pPool )
{
	apr_array_header_t *targets = apr_array_make( in_pPool, (int)in_rPathList.size(), sizeof(const char *) );

	for (const auto& path : in_rPathList)
		APR_ARRAY_PUSH( targets, const char * ) = apr_pstrdup( in_pPool, GetSVNPath(path, in_pPool) );
...
{
	if (in_rStatusList.empty())
		return false;

	for (const auto& status : in_rStatusList)
	{
		if (!IsOperationValid(in_operationID, status, in_bMultipleSelection))
			return false;
	}
...
svn_error_t* SubversionSourceControl::CancelCallback( AK::Wwise::ISourceControlUtilities* in_pUtilities )
{
	...
	if ( in_pUtilities->IsCancelRequested() || (pProgress && pProgress->IsCanceled()) )
	{
		AFX_MANAGE_STATE( AfxGetStaticModuleState() );
