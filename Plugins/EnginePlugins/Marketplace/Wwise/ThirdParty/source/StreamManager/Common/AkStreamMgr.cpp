...
namespace AK
{
	namespace StreamMgr
	{
		inline void MonitorFileOpenError(AKRESULT in_eRes, AkFileID in_fileID, const AkOSChar* in_pszFilePath)
		{
#ifndef AK_OPTIMIZED
			//Convert file ID to string
			AkOSChar fileID[25];
			AK_OSPRINTF(fileID, 25, AKTEXT("with file ID: %u"), in_fileID);
			MonitorFileOpenError(in_eRes, fileID, in_pszFilePath);
#endif
		}

		inline void MonitorFileOpenError(AKRESULT in_eRes, const AkOSChar * in_pszFileName, const AkOSChar* in_pszFilePath)
		{
#ifndef AK_OPTIMIZED
			// Convert to utf-8			
			char* utf8FileName = CONVERT_OSCHAR_TO_CHAR(in_pszFileName, utf8FileName);
			char* utf8Path = CONVERT_OSCHAR_TO_CHAR(in_pszFilePath, utf8Path);

			// Monitor error.
			if (AK_Success == in_eRes)
			{
				AKASSERT(!"Invalid file size: fileDesc.iFileSize <= 0");	// Low-level IO implementation error.
				AK::Monitor::PostCodeVarArg(AK::Monitor::ErrorCode_InvalidFileSize, AK::Monitor::ErrorLevel_Error, AK::Monitor::MsgContext(), utf8FileName, utf8Path);
			}
			else if (AK_FileNotFound == in_eRes)
			{
				AK::Monitor::PostCodeVarArg(AK::Monitor::ErrorCode_FileNotFound, AK::Monitor::ErrorLevel_Error, AK::Monitor::MsgContext(), utf8FileName, utf8Path);
			}
			else
			{
				AK::Monitor::PostCodeVarArg(AK::Monitor::ErrorCode_CannotOpenFile, AK::Monitor::ErrorLevel_Error, AK::Monitor::MsgContext(), utf8FileName, utf8Path);
			}
#endif
		}
	}
...
#ifndef AK_OPTIMIZED
static inline bool IsLocalizedFallbackExpected(AkFileSystemFlags * in_pFSFlags)
{
	return
		in_pFSFlags &&
		in_pFSFlags->bIsLanguageSpecific &&
		in_pFSFlags->uCompanyID == AKCOMPANYID_AUDIOKINETIC &&
		(AK::IsBankCodecID(in_pFSFlags->uCodecID) || in_pFSFlags->uCodecID == AKCODECID_FILE_PACKAGE);
}
#endif
...
AKRESULT AkDeferredOpenData::Execute( AkFileDesc & io_fileDesc )
{
		...
		|| !bSyncOpen)
	{
		...
		// Monitor error.
		AkOSChar* exploredPath = (AkOSChar*)AkAlloca(sizeof(AkOSChar) * AK_MAX_PATH);
		//Make sure the out_searchedPath is empty
		memset(exploredPath, 0, AK_MAX_PATH);
		if (bByString)
		{
			CAkStreamMgr::GetFileLocationResolver()->OutputSearchedPaths(eResult, pszFileName, pFlags, eOpenMode, exploredPath, AK_MAX_PATH);
			MonitorFileOpenError(eResult, pszFileName, exploredPath);
		}
		else
		{
			CAkStreamMgr::GetFileLocationResolver()->OutputSearchedPaths(eResult, fileID, pFlags, eOpenMode, exploredPath, AK_MAX_PATH);
			MonitorFileOpenError(eResult, fileID, exploredPath);
		}
...
AKRESULT CAkStreamMgr::PerformIO()
{
	...
	return AK_NotCompatible; // This should only be called from single-threaded environments.
#else
	for ( AkUInt32 uDevice = 0; uDevice < m_arDevices.Length(); uDevice++ )
	{
		if (m_arDevices[uDevice])
		{
			m_arDevices[uDevice]->PerformAllPendingIO();
		}
	}
	return AK_Success;
#endif
	...
	)
{
		...
		|| (pFileDesc->iFileSize <= 0 && bSyncOpen && in_eOpenMode == AK_OpenModeRead))
	{
		...
		if (IsLocalizedFallbackExpected(in_pFSFlags))
			 return eRes;

		AkOSChar* exploredPath = (AkOSChar*)AkAlloca(sizeof(AkOSChar) * AK_MAX_PATH);
		//Make sure the out_searchedPath is empty
		memset(exploredPath, 0, AK_MAX_PATH);
		m_pFileLocationResolver->OutputSearchedPaths(eRes, in_pszFileName, in_pFSFlags, AK_OpenModeRead, exploredPath, AK_MAX_PATH);
		MonitorFileOpenError(eRes, in_pszFileName, exploredPath);
	...
	)
{
		...
		|| (pFileDesc->iFileSize <= 0 && bSyncOpen && in_eOpenMode == AK_OpenModeRead))
	{
		...
		//Make sure the out_searchedPath is empty
		memset(exploredPath, 0, AK_MAX_PATH);
		CAkStreamMgr::GetFileLocationResolver()->OutputSearchedPaths(eRes, in_fileID, in_pFSFlags, in_eOpenMode, exploredPath, AK_MAX_PATH);
		MonitorFileOpenError(eRes, in_fileID, exploredPath);
	...
	)
{
		...
		|| (bSyncOpen && pFileDesc->iFileSize <= 0))
	{
		...
		//Make sure the out_searchedPath is empty
		memset(exploredPath, 0, AK_MAX_PATH);
		CAkStreamMgr::GetFileLocationResolver()->OutputSearchedPaths(eRes, in_fileID, in_pFSFlags, AK_OpenModeRead, exploredPath, AK_MAX_PATH);
		MonitorFileOpenError(eRes, in_fileID, exploredPath);
